...
export default function ReviewerReview() {
  ...
  useEffect(() => {
    if (!id || sessionStatus !== 'authenticated' || session?.user?.role !== 'REVIEWER') return;
    
    const fetchData = async () => {
      setLoading(true);
      try {
        // Fetch the review assignment
        const assignmentResponse = await axios.get(`/api/reviewer/assignments/${id}`);
        const reviewData = assignmentResponse.data;
        setReviewAssignment(reviewData);
        
        // Fetch the application details
        const applicationResponse = await axios.get(`/api/applications/${reviewData.applicationId}`);
    ...
    return (
      <Layout title="Application Review | Error">
        <div className="container mx-auto px-4 py-8">
          ...
          <Button className="mt-4" onClick={() => router.push('/reviewer-dashboard')}>
    ...
    return (
      <Layout title="Application Review | Not Found">
        <div className="flex h-screen flex-col items-center justify-center">
          ...
          <p className="mt-2 text-muted-foreground">The review assignment you're looking for doesn't exist or you don't have permission to access it.</p>
          <Button className="mt-6" onClick={() => router.push('/reviewer-dashboard')}>
            <ArrowLeft className="mr-2 h-4 w-4" />
          ...
          <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
            <div className="flex flex-col items-start gap-y-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex items-center space-x-3">
                <div>
                  <span className="mr-2 text-sm text-muted-foreground">Application Status:</span>
                ...
                <CardContent>
                  <div className="flex flex-wrap gap-4">
                    {application.pitchDeckUrl && (
                      <Button variant="outline" onClick={() => window.open(application.pitchDeckUrl, '_blank')}>
                        <Download className="mr-2 h-4 w-4" />
                        View Pitch Deck
                      </Button>
                    )}
                    {application.financialsUrl && (
                      <Button variant="outline" onClick={() => window.open(application.financialsUrl, '_blank')}>
                        <Download className="mr-2 h-4 w-4" />
                        View Financials
                      </Button>
                    )}
                  ...
                  <div className="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        disabled={isViewMode || reviewAssignment.status === 'COMPLETED'}
                      />
                    ...
                    <div className="flex">
                      <div className="ml-3">
                        ...
                        <div className="mt-2 text-sm text-gray-500 dark:text-gray-400">
                          <p>
                            This is the average of all four scores and will be the main rating displayed for this application.
                          </p>
                        </div>
                      </div>
                    </div>
